...
export interface RealtimeTicketPayload { userId: number; namespace: string; exp: number; }
...
export function createRealtimeTicket(userId: number, secret: string, namespace = "main", ttlSeconds = 60, nowMs = Date.now()): string {
    ...
    if (!NAMESPACE_PATTERN.test(namespace)) throw new Error("namespace is invalid");
    const payload = Buffer.from(JSON.stringify({ userId, namespace, exp: Math.floor(nowMs / 1000) + ttlSeconds }), "utf8").toString("base64url");
...
export function verifyRealtimeTicket(
    ...
    try {
        ...
        const namespace = typeof decoded.namespace === "string" ? decoded.namespace : "main";
        if (typeof userId !== "number" || !Number.isSafeInteger(userId) || userId <= 0 ||
            typeof exp !== "number" || !Number.isSafeInteger(exp) || exp <= Math.floor(nowMs / 1000) ||
            !NAMESPACE_PATTERN.test(namespace) || !allowedNamespaces.has(namespace)) return null;
        return { userId, namespace, exp };
    } catch { return null; }
}
